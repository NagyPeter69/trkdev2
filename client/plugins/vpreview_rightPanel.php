...
<div id='rightPanel'>
	<? if( $check[0][8] == "pair" ) { ?>
	<div title='<?= $lang["flatplan"]["pagepair_tip"] ?>' class='rightPanelElement' onclick="toggleTool('pair')" func='pair' style='height: 30px;'>
		<img class='normal' id="pair" src='plugins/images/pair.png'>
		<img class='hover' id="pair" src='plugins/images/pair_hover.png'>
	</div>
	<? } ?>
	<div title='<?= $lang["flatplan"]["pagewide_tip"] ?>' style='height: 30px; display: block;' id="widePage_div" class='rightPanelElement' onclick="toggleTool('widePage')" func='widePage'>
	...
	<div  title='<?= $lang["flatplan"]["pagesingle_tip"] ?>'  style='height: 31px; display: none; height: 31px !important;' id="single_div" class='rightPanelElement' onclick="toggleTool('single')" func='single'>
		<img class='normal' id="single" src='plugins/images/single.png'>
		<img class='hover' id="single" src='plugins/images/single_hover.png'>
	</div>
	<div  title='<?= $lang["flatplan"]["zoom_tip"] ?>'  style='height: 31px;' class='rightPanelElement' onclick="advancedTool('magnify')" func='magnify'>
	...
	<div  title='<?= $lang["flatplan"]["pan_tip"] ?>'  style='height: 33px;' class='rightPanelElement' func='hand'>
	...
	<div title='<?= ( $user[0][15] == trimbox ? $lang["flatplan"]["crop_tip"] : $lang["flatplan"]["trim_tip"] ) ?>' style='height: 33px; display: <?= ( $user[0][34] == "1" ? "block" : "none" ) ?>;' class='rightPanelElement' onclick="toggleBox( this )" func='boxes'>
		<?
	...
	<div  title='<?= $lang["flatplan"]["galery_tip"] ?>'  style='height: 30px;' class='rightPanelElement' onclick="toggleBar()">
	...
	<div  title='<?= $lang["flatplan"]["measure_tip"] ?>'  style='height: 20px;' class='rightPanelElement' onclick="advancedTool('measure')" func='measure'>
	...
	<div  title='<?= $lang["flatplan"]["sample_tip"] ?>'  style='height: 32px; display: <?= ( $user[0][34] == "1" ? "block" : "none" ) ?>;' class='rightPanelElement' onclick="advancedTool('colorPicker')" func='colorPicker'>
	...
	<div  title='<?= $lang["flatplan"]["compare_tip"] ?>'  style='height: 29px;' class='rightPanelElement' onclick="compareToggle()" func='compare'>
	...
	<div  title='<?= $lang["flatplan"]["comment_tip"] ?>'  style='height: 26px;' class='rightPanelElement panelCorrection' onclick='panelElementClick( "square", "square" );' func='square'>
		<img class='normal' id="square" src='plugins/images/square.png'>
		<img class='hover' id="square" src='plugins/images/square_hover.png'>
	</div>
	<div  title='<?= $lang["flatplan"]["comment_tip"] ?>'  style='height: 28px;' class='rightPanelElement panelCorrection' onclick='panelElementClick( "circle", "circle" );' func='circle'>
		<img class='normal' id="circle" src='plugins/images/circle.png'>
		<img class='hover' id="circle" src='plugins/images/circle_hover.png'>
	</div>
	<div  title='<?= $lang["flatplan"]["comment_tip"] ?>'  style='height: 33px;' class='rightPanelElement panelCorrection' onclick='panelElementClick( "dot", "dot" );' func='dot'>
		<img class='normal' id="dot" src='plugins/images/dot.png'>
		<img class='hover' id="dot" src='plugins/images/dot_hover.png'>
	</div>
	<div  title='<?= $lang["flatplan"]["commenthide_tip"] ?>'  id='showComment' style='height: 17px !important; margin-top: 3px; margin-bottom: 3px;' class='rightPanelElement panelCorrection' onclick='toggleComments( "all" );' func='dot'>
	...
	<div style='display: <?= ( $user[0][34] == "1" ? "block" : "none" ) ?>;'>
		<div class='rightPanelSeparator'>
			<img src='plugins/images/spacer.png'>
		</div>	
		<div  title='<?= $lang["flatplan"]["cyan_tip"] ?>'  style='height: 31px;' class='rightPanelElement' func='cyan' onclick="toggleColor('cyan')">
			<img class='normal' id="cyan" src='plugins/images/cyanOn.png'>
			<img class='hover' id="cyan" src='plugins/images/cyan_hover.png'>
		</div>
		<div  title='<?= $lang["flatplan"]["magenta_tip"] ?>'  style='height: 31px;' class='rightPanelElement' func='magenta' onclick="toggleColor('magenta')">
		...
		<div  title='<?= $lang["flatplan"]["yellow_tip"] ?>'  style='height: 31px;' class='rightPanelElement' func='yellow' onclick="toggleColor('yellow')">
			<img class='normal' id="yellow" src='plugins/images/yellowOn.png'>
			<img class='hover' id="yellow" src='plugins/images/yellow_hover.png'>
		</div>
		<div  title='<?= $lang["flatplan"]["black_tip"] ?>'  style='height: 31px;' class='rightPanelElement' func='kblack' onclick="toggleColor('kblack')">
...
function renderComparePages( file, place, nr ) {
	if( previousCompareOperation != "diff" || nr == 1 || nr == 4 ) {
		...
		if( pages == "1" ) {
			...
			// see preview_rightPanel.php's identical fix for the full
			// explanation.
		...
		$.ajax	({
			url:"engine/compare.php?op=render&visitor="+user+"&cMode="+cMode+"&zoom="+zoom+"&nr="+nr+"&to=compare_"+nr,
			type: "POST",
...
function loadComparePages( file, place, nr, sbsf ) {
	if( previousCompareOperation != "diff" || nr == 1 || nr == 4 ) {
		...
		$.ajax	({
			url:"engine/compare.php?op=loadbg&visitor="+user+"&page="+page+"&pages="+pages+"&id="+pub+"&pack_id="+pack_id+"&alter="+PageType+"&state="+place,
			type: "POST",
...
function loadDiff( a, b, nr, place ) {
	...
	$.ajax	({
		url:"engine/compare.php?op=loaddiff&visitor="+user+"&a="+a+"&b="+b+"&type="+ compare["cBox"][2],
		type: "POST",
...
function compareToggle( force ) {
	if( force == "force" ) {
		...
		$.ajax	({
			url:"engine/compare.php?op=loadpanel&visitor="+user+"&page="+page+"&pages="+pages+"&id="+pub+"&pack_id="+pack_id+"&alter="+PageType,
			type: "POST",
...
function posPreview2() {
	var higher = ( file[0]["Height"] > file[1]["Height"] ? file[0] : file[1] );
	var atm = {
		"height" : pixel( ( parseFloat( higher['Top'] )-parseFloat( higher['Bottom'] )) ),
...
function posPreview() {
	...
	var def = {
		"height" : pixel( ( parseFloat( higher['Top'] )-parseFloat( higher['Bottom'] )), 100 ),
	...
	if( isNaN( atm.height ) ) {
		atm.height = pixel( ( parseFloat( file[0]['Top'] )-parseFloat( file[0]['Bottom'] )) );
		}
...
function toggleBar() {
	...
	if( parseInt( bar.css("left") ) >= 0 ) {
		...
		$("#rightArrow").animate({ left: (pagesLeft+parseInt($(".pages").outerWidth())-19)+"px" }, 200 );
		$("#leftArrow_hover").animate({ left: (pagesLeft-36)+"px" }, 200 );
		$("#rightArrow_hover").animate({ left: (pagesLeft+parseInt($(".pages").outerWidth())-19)+"px" }, 200 );
		...
		$(".pages").animate({ left: pagesLeft+"px" }, 200 );
		$("#leftArrow").animate({ left: (pagesLeft-36)+"px" }, 200 );
		$("#rightArrow").animate({ left: (pagesLeft+parseInt($(".pages").outerWidth())-13)+"px" }, 200 );
		$("#leftArrow_hover").animate({ left: (pagesLeft-36)+"px" }, 200 );
		$("#rightArrow_hover").animate({ left: (pagesLeft+parseInt($(".pages").outerWidth())-13)+"px" }, 200 );
		var temp = pagesLeft+parseInt($(".pages").outerWidth());
		var maradt = (footer-temp)/2;
		status = ( (temp+maradt)-(parseInt($(".status2").outerWidth())/2 ) );
		$(".status2").animate({ left: status+"px" }, 200 );
...
function placeBox( force, source, from ) {
	...
	setTimeout( function(){
		...
		$(".pagePreview").fadeIn( 200,function(){
			if( force != false ) {
...
function reloadBG( switchTo, from ) {
	...
	$.ajax	({
		url:"engine/vflatplan_ajax.php?op=reloadbg&tag="+tag+"&zoom="+zoom+"&clk="+clk+"&id="+pub+"&p="+page+"&fpFilter="+fpFil+"&hash=<?= $_GET["hash"] ?>",
		type: "POST",
		...
		success:function( data ) {
			...
			if( data[0] != "error" ) {
				...
				if( file[0]['Left'] == "0" ) {
					...
					defaultSizesTrim['Height'] = parseFloat(file[0]['Top'])-(2*trimbox[0]['Bottom']);
					}
				...
				setTimeout( function(){ placeBox('force', '', from ); }, 50 ); 
				}
			else {
				$("#renderCounter").val( ( parseInt( $("#renderCounter").val() )-1 ) ).trigger("onchange");
				$("#errorInfo").html( "hiba" );
...
function createBoxes() {
	...
	if( cBox[2] == "trimbox" ) {
		...
		if( pages > 1 ) {
			var maxheight = ( file[0]["Height"] > file[1]["Height"] ? pixel(file[0]["Top"]-file[0]["Bottom"]) : pixel(file[1]["Top"]-file[1]["Bottom"]) );
			var corr = ( maxheight - pixel(file[0]["Top"]-file[0]["Bottom"]) ) / 2;		
			margin.bottom += corr;
			}
		...
		if( pages > 1 ) {
			...
			var maxheight = ( file[0]["Height"] > file[1]["Height"] ? pixel(file[0]["Top"]-file[0]["Bottom"]) : pixel(file[1]["Top"]-file[1]["Bottom"]) );
			var corr = ( maxheight - pixel(file[1]["Top"]-file[1]["Bottom"]) ) / 2;	
	...
	if( cBox[2] == "mediabox" ) {
		if( pages > 1 ) {
			correction = {
				"Bottom": ( parseFloat(trimbox[0]["Bottom"]) != 0 ) ? parseFloat(trimbox[0]["Bottom"])-tempBleed-tempKifuto : 0,
				"Left": ( parseFloat(trimbox[0]["Left"]) != 0 ) ? parseFloat(trimbox[0]["Left"])-tempBleed-tempKifuto : 0
				}
			margin = {
				...
				"bottom": Math.floor( pixel( parseFloat(bleedbox[0]["Bottom"]) ) )
				}
		...
		if( pages > 1 ) {
			...
			margin = {
				...
				"bottom": Math.floor( pixel( trimbox[0]["Bottom"] ) )
				}
			...
			tempWidth = Math.floor( pixel( ( parseFloat( trimbox[0]["Right"] )-parseFloat( trimbox[0]["Left"] ) ) ) )-1.2;
			tempHeight = Math.floor( pixel( ( parseFloat( trimbox[0]["Top"] )-parseFloat( trimbox[0]["Bottom"] ) ) ) )-1.2;
			}
		...
		if( pages > 1 ) {
			...
			margin = {
				...
				"bottom": Math.floor( pixel( parseFloat(bleedbox[1]["Bottom"]) ) )
				}
			var maxheight = ( file[0]["Height"] > file[1]["Height"] ? pixel(file[0]["Top"]-file[0]["Bottom"]) : pixel(file[1]["Top"]-file[1]["Bottom"]) );
			var corr = ( maxheight - pixel(file[1]["Top"]-file[1]["Bottom"]) ) / 2;	
			...
			margin = {
				...
				"bottom": Math.floor( pixel( trimbox[1]["Bottom"] ) )
				}
...
function pickerHandle( x, y ) {
	...
	if( cFile != "" ) {
		$.ajax	({
			url:"engine/vflatplan_ajax.php?op=colorPick&mode="+mode,
			type: "POST",
...
var trim_tip = "<?= $lang['flatplan']['trim_tip'] ?>";
var crop_tip = "<?= $lang['flatplan']['crop_tip'] ?>";
...
function toggleBox( img ) {
	...
	switch( current ) {
		case 'mediabox':
			switchTo = 'trimbox';
			$(img).attr("title", crop_tip );
			break;
		case 'trimbox':
			switchTo = 'mediabox';
			$(img).attr("title", trim_tip );
			break;
		}
	removeAdvancedTool();
...
function toggleTool( tool ) {
	...
	if( tool == "widePage" ) {
		...
		$('.pagePreview').fadeOut( 200,function(){
			...
			$.ajax	({
				url:"engine/vflatplan_ajax.php?op=saveTool",
				type: "POST",
				data: { data: data },
				dataType: 'json',
				success:function( data ) {
...
function togglePantone( id ) {
	...
	if( !ajaxDisabled ) {
		...
		disableZoom = true;
 		$.ajax	({
			url:"engine/vflatplan_ajax.php?op=reloadbg&id="+pub+"&pack_id="+pack_id+"&p="+page+"&alter="+PageType,
			type: "POST",
			data: { colors: colors },
			dataType: 'json',
			success:function( data ) {
