...
<div id='rightPanel'>
	<div style='height: 31px; height: 31px !important;' id="single_div" class='rightPanelElement' onclick="toggleTool('single')" func='single'>
	...
	<div style='height: 33px; display: none;' class='rightPanelElement' onclick="toggleBox( this )" func='boxes'>
		<?
	...
	<div style='display: <?= ( $user[0][26] == "1" ? "block" : "none" ) ?>;'>
		<div class='rightPanelSeparator'>
...
function reloadBG( switchTo, from ) {
	...
	console.log( "engine/flatplan_ajax.php?op=advert_preview_reloadbg&id=<?= $_GET['id'] ?>&p=<?= $_GET['p']?>" );
	$.ajax	({
		url:"engine/flatplan_ajax.php?op=advert_preview_reloadbg&id=<?= $_GET['id'] ?>&p=<?= $_GET['p']?>",
		type: "POST",
...
function createBoxes() {
	...
	var tempBleed_ = pixel( tempBleed );
	...
	if( cBox[2] == "mediabox" ) {
		correction = {
			"Bottom": parseFloat(trimbox[0]["Bottom"])-tempBleed_-tempKifuto,
			"Left": parseFloat(trimbox[0]["Left"])-tempBleed_-tempKifuto
			}
		...
		tempWidth = Math.floor(pixel( ( parseFloat( bleedbox[0]["Right"] )-parseFloat( bleedbox[0]["Left"] ) ) ) )-3;
		tempHeight = Math.floor( pixel( ( parseFloat( bleedbox[0]["Top"] )-parseFloat( bleedbox[0]["Bottom"] ) ) ) )-3;
		...
		margin = {
			"left": Math.floor( margin["left"] + tempBleed_ ),
			"right": Math.floor( margin["right"] + tempBleed_ ),
			"top": Math.floor( margin["top"] + tempBleed_ ),
			"bottom": Math.floor( margin["bottom"] + tempBleed_ )
			}
		...
		tempWidth = tempWidth - 2*tempBleed_;
		tempHeight = tempHeight - 2*tempBleed_;
		...
		if( safety_on == "1" ) {
			...
			margin = {
				"left": Math.floor( margin["left"] + tempSafety ),
				"right": Math.floor( margin["right"] + tempSafety ),
				"top": Math.floor( margin["top"] + tempSafety ),
				"bottom": Math.floor( margin["bottom"] + tempSafety )
				}
			
			tempWidth = tempWidth - 2*tempSafety;
			tempHeight = tempHeight - 2*tempSafety;
...
function pickerHandle( x, y ) {
	...
	if( cFile != "" ) {
		$.ajax	({
			url:"engine/flatplan_ajax.php?op=advert_colorPick&mode="+mode,
			type: "POST",
...
function measureHandle() {
	...
	infoText += "</table>";
	infoText += "<div style='padding-top: 1px; padding-left: 3px;'>"+lang['percent']+" "+Math.abs(precise_round( measureInfo.percent, 2 ) )+" "+lang['percent2']+"</div>";
...
function toggleTool( tool ) {
	...
	if( tool == "widePage" ) {
		...
		$('.pagePreview').fadeOut( 200,function(){
			...
			$.ajax	({
				url:"engine/flatplan_ajax.php?op=saveTool",
				type: "POST",
				data: { data: data },
				dataType: 'json',
				success:function( data ) {
...
function togglePantone( id ) {
	...
	if( !ajaxDisabled ) {
		...
		disableZoom = true;
 		$.ajax	({
			url:"engine/flatplan_ajax.php?op=reloadbg&id="+pub+"&pack_id="+pack_id+"&p="+page+"&alter="+PageType,
			type: "POST",
			data: { colors: colors },
			dataType: 'json',
			success:function( data ) {
