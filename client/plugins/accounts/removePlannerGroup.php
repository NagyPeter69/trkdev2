...
<div>
	<div class='panelTitle'>Remove Existing Planner Group</div>
	...
	<table class='panelTable' id='job_names' cellspacing='0' cellpadding='0'>
		<tbody>
			...
			$groups = array();
			if( $user[0][8] == 2 ) {
				$sql = sql_get( "calendar_groups", "1 order by `name` ASC", "*" );
				}
			else {
				$sql = sql_get( "calendar_groups", "publisher='".$user[0][4]."' order by `name` ASC", "*" );
				}
			...
			<tr>
				...
				<td colspan="2" align="left" style="width: 210px; padding-top: 10px;">
					<div onclick="closePanel( 'accounts_removePlannerGroup', 'back')" style="margin-left: 2px;" class="panelButton">Cancel</div>
					<div onclick="menuApply( 'accounts', 'removePlannerGroup')" style="margin-left: 20px;" class="panelButton">Apply</div>
				</td>
			</tr>		
		</tbody>
	</table>		
