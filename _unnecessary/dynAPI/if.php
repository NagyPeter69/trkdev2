...
$pdf->Append();
		...
		$pdf->WriteText( coord(5+$corr1x), coord(20.4+$corr1y), $_POST["first_name"]." ".$_POST["last_name"] );

		$pdf->SetFont('DIN Next W1G Light', dynapdf::fsRegular, 7.0, true, dynapdf::cpUnicode);
		$pdf->WriteText( coord(6.1+$corr1x), coord(25+$corr1y), $_POST["titulus"] );
		
		$pdf->SetFillColor( $pdf->CMYK( 0, 0, 0, 178.5 ) );
		$pdf->SetFont('DIN Next W1G Light', dynapdf::fsRegular, 7.0, true, dynapdf::cpUnicode);
		...
		if( empty( $_POST["fiok2"] ) ) {
			if( trim($_POST["phone"]) != "+36" ) {
				...
				$pdf->WriteText( coord(5+$corr1x), coord(37.3+$corr1y), $_POST["fiok"] );
				$pdf->WriteText( coord(5+$corr1x), coord(40.3+$corr1y), $_POST["email"] );
				$pdf->WriteText( coord(5+$corr1x), coord(43.3+$corr1y), "www.otpbank.hu" );				
				}
			}
			...
			if( trim($_POST["phone"]) != "+36" ) {
				...
				$pdf->WriteText( coord(5+$corr1x), coord(32.8+$corr1y), "OTP Bank Nyrt." );
				$pdf->WriteText( coord(5+$corr1x), coord(35.8+$corr1y), $_POST["fiok"] );
				$pdf->WriteText( coord(5+$corr1x), coord(38.8+$corr1y), $_POST["fiok2"] );
				$pdf->WriteText( coord(5+$corr1x), coord(41.8+$corr1y), $_POST["email"] );
				$pdf->WriteText( coord(5+$corr1x), coord(44.8+$corr1y), "www.otpbank.hu" );
				}
			}
