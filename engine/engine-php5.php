...
include("/var/www/intra/engine/constans.php");
...
function arrowChecker( $id, $page ) {
	$current = sql_aget( "flatplan_planner", "pub_id='".$id."' AND pos = ".$page."", "*" );
	$lastpage = sql_aget( "flatplan_planner", "pub_id='".$id."' AND name='".$current[0]["name"]."' order by pos DESC LIMIT 1", "*" );
	
	$color = "";
	if( $current[0]["workerID"] != "0" ) {
		$worker = sql_aget( "accounts", "id='".$current[0]["workerID"]."'", "*" );
		$color = $worker[0]["color"];
		}
	
	$dot = "";
	if( $current[0]["text"] == "1" ) {
		$dot .= "<div class='dot_text'>".( $current[0]["have_text"] == "0" ? "<div class='dot_required'></div>" : "" )."</div>";
		}
	if( $current[0]["image"] == "1" ) {
		$dot .= "<div class='dot_image'>".( $current[0]["have_image"] == "0" ? "<div class='dot_required'></div>" : "" )."</div>";
		}
	if( $current[0]["other"] == "1" ) {
		$dot .= "<div class='dot_other'>".( $current[0]["have_other"] == "0" ? "<div class='dot_required'></div>" : "" )."</div>";
		}		
	
	$txt = "";		
	if( $lastpage[0]["pos"] == $page && $current[0]["type"] != "ad" && $current[0]["type"] != "promo" ) {
		$txt .= "<div onclick='settingsPanel(\"flatplan_worker\", undefined, \"".$current[0]["id"]."\" )' data-id='".$current[0]["id"]."' class='fp-user-icon'>
					<i class='fp-icons fas fa-user' style='".( !empty( $color ) ? "color: ".$color : "" )."'></i>
				</div>
				
				<div class='dotBox'>".$dot."</div>";		
		}
	
	if( $current[0]["type"] == "promo" ) {
		$txt .= "<div onclick='settingsPanel(\"flatplan_worker\", undefined, \"".$current[0]["id"]."\" )' data-id='".$current[0]["id"]."' class='fp-user-icon' style='right: 4px;'>
					<i class='fp-icons fas fa-user' style='".( !empty( $color ) ? "color: ".$color : "" )."'></i>
				</div>
				
				<div class='dotBox' style='right: 4px;'>".$dot."</div>";	
...
function invoicingTESZT( $pubID ) {
	...
	if( $process == "Full" or $process == "Hybrid" ) {
		...
		$proofs = sql_aget( "pageinfo", "code='".$magazine[0]["code"]."' AND issue='".$pub[0]["code"]."' AND proofCounter != '0' AND type != 'ad' GROUP BY page", "*" );
...
function calendarOrderbox( $order, $bottom, $date, $mags, $boxCount = false ) {
	global $rights;
	...
	return "<div ondragstart='dragStart(event)' draggable='".(  $rights["calendar_realdates"] ? "true" : "false" )."' magid='".$order["magazine_id"]."' class='draggable printOrder orderTile' style='background-color: #".$magcolor."; color: ".$fontcolor."; bottom: ".$bottom."px;' magCode='".$order["magCode"]." ".$order["code"]."' plannerId='".$order["id"]."'>
					...
					if( $user[0][29] != "sales") {
						if( $key == "0" ) {
							...
							if( $rights["calendar_realdates"] == false  ) {
										...
										if( in_array( $print_check[$p]["magazine_id"], $magazines ) ) {
											...
											if( $mag[0]["dayshift"] == $c ) {
												...
												if( $nap != "Sat" && $nap != "Sun" ) {
													...
													$row .= calendarOrderbox( $print_check[$p], $bottom, $check_date, $m, $boxCount );
													}
								...
								if( $nap == "Fri" && !empty( $month[$i] ) ) {
									$cd[0] = date( "Y-m-d" , strtotime( '+1 days', strtotime( $check_date ) ) );
									$cd[1] = date( "Y-m-d" , strtotime( '+2 days', strtotime( $check_date ) ) );
									$cd[2] = date( "Y-m-d" , strtotime( '+3 days', strtotime( $check_date ) ) );
									$cd[3] = date( "Y-m-d" , strtotime( '+4 days', strtotime( $check_date ) ) );								
									...
									$cd[0] = date( "Y-m-d" , strtotime( '+2 days', strtotime( $check_date ) ) );
					...
					if( $user[0][29] != "print") {
						...
						for( $p = 0; $p < count( $print_check ); $p++ ) {
							if( in_array( $print_check[$p]["magazine_id"], $magazines ) ) {
								...
								$magcolor = adjustBrightness( $magcolor[0][0], -40 );
								$fontcolor = fontcolor( $magcolor );
								$bottom = 16 * $counter;
...
function produkcioSendmailAttach( $subject, $body, $to, $attach ) {
	if( is_file( '/var/www/intra/client/plugins/phpmail/PHPMailerAutoload.php' ) ) {
		require_once('/var/www/intra/client/plugins/phpmail/PHPMailerAutoload.php');
		}
	else {
	...
	$mail->Username = "produkcio";
	$mail->Password = "oickudorp";
	...
	$mail->SetFrom ('produkcio@colorcom.hu', 'Colorcom Prepress');
...
function adThumbCreate2( $path, $file, $to ) {
	$terminal = "/var/www/intra/client";
	$trim = getPDFBox( "Trimbox ", "engine/switch/".$file );
	$trim = $trim["Trimbox"];
	...
	$command = './r3 -binary -mode:RENDER -left:'.($trim[0]).' -right:'.($trim[2]-14.1732).' -bottom:'.$trim[1].' -top:'.($trim[3]-14.1732).' -width:'.$tWidth.' -height:'.$tHeight.' -tprofile:sRGB_Color_Space_Profile.icc -sprofile:ISOcoated_v2_eci.icc '.$from.' $@ >'.$to.' 2>&1';
...
function thumbCreate2( $file, $pageWidth ) {
	...
	$command = './r3 -binary -mode:RENDER -left:'.$trim[0].' -right:'.$trim[2].' -bottom:'.$trim[1].' -top:'.$trim[3].' -width:'.$width.' -height:'.$height.' -tprofile:sRGB_Color_Space_Profile.icc -sprofile:ISOcoated_v2_eci.icc '.$from.' $@ >'.$to.' 2>&1';
...
function colorPick( $pdfPath, $x, $y ) {
	$command = './r3 -mode:MEASURE -x:'.$x.' -y:'.$y.' -tprofile:ISOcoated_v2_eci.icc '.$pdfPath.' 2>&1';
	$command = shell_exec('
			cd /var/www/intra/client/engine/r3 2>&1;
			'.$command.';
			');
...
function getColorTitles( $pdfPath ) {
	$titles = array();
	$command = './r3 -mode:MEASURE -x:596 -y:760 -d:1 -r:600 -tprofile:ISOcoated_v2_eci.icc '.$pdfPath.' 2>&1';
	$command = shell_exec('
			cd /var/www/intra/client/engine/r3 2>&1;
			'.$command.';
			');
...
function checkPageStatus( $page, $id, $pack_id, $alter, $state = '' ) {
	$issue = sql_get( 'publications', 'id="'.$id.'"', '*' );
	...
	$checker = sql_get( 'pageinfo', 'issue="'.$issue[0][10].'" AND '.$alter.' AND state="'.$state.'" AND code="'.$magazine[0][3].'" AND page="'.$page.'"', '*' );
	return array( $checker[0][4], $checker[0][0] );
	}

function checkPagePair( $id, $pack_id, $page, $tag, $alter = '', $prefix = '' ) {
	switch( $prefix ) {
		case "prev":
			$prefix = ".";
			break;
	...
	$checker = sql_get( 'pageinfo', 'issue="'.$issue[0][10].'" AND code="'.$magazine[0][3].'" AND page="'.( intval( $page )+1 ).'"', '*' );
...
function getPDFBox( $box, $file ) {
	...
	$terminal = "/var/www/intra/client";
	$command = "./r3 -mode:GETDATA -metadata ".$terminal."/".$file;
...
function PDFtoImage_remote( $sizes, $from, $to, $colors = "" ) {
	$data["pdf"] = file_get_contents( $from );
	$data["Left"] = $sizes["Left"];
	$data["Right"] = $sizes["Right"];
	$data["Top"] = $sizes["Top"];
	$data["Bottom"] = $sizes["Bottom"];
	...
	$url = 'http://'.RENDERIP.'/r3API/pdftoimage.php';
	$ch = curl_init();
	...
	$ru = microtime(true);
	echo "Rendering time: " . rutime($ru, $rustart, "stime") ." ms ";
...
function PDFtoImage_Measure( $sizes, $from, $to, $colors = "" ) {
	...
	if( $colors != "" ) {
		...
		$command = './r3 -binary -mode:RENDER -left:'.$sizes["Left"].' -right:'.$sizes["Right"].' -bottom:'.$sizes["Bottom"].' -top:'.$sizes["Top"].' -width:'.$sizes["Width"].' -height:'.$sizes["Height"].' -colors:'.$color.' -tprofile:sRGB_Color_Space_Profile.icc -sprofile:ISOcoated_v2_eci.icc '.$from.' > '.$to.'';
		}
	else {
		$command = './r3 -binary -mode:RENDER -left:'.$sizes["Left"].' -right:'.$sizes["Right"].' -bottom:'.$sizes["Bottom"].' -top:'.$sizes["Top"].' -width:'.$sizes["Width"].' -height:'.$sizes["Height"].' -tprofile:sRGB_Color_Space_Profile.icc -sprofile:ISOcoated_v2_eci.icc '.$from.' > '.$to.'';
		}
	
	echo $command."<br>";
	
	$command = shell_exec('
			cd /var/www/intra/client/engine/r3
			'.$command.';
			');	
...
function PDFtoImage_( $sizes, $from, $to, $colors = "" ) {
	if( $colors != "" ) {
		...
		$command = './r3 -binary -mode:RENDER -left:'.$sizes["Left"].' -right:'.$sizes["Right"].' -bottom:'.$sizes["Bottom"].' -top:'.$sizes["Top"].' -width:'.$sizes["Width"].' -height:'.$sizes["Height"].' '.$from.' -tprofile:sRGB_Color_Space_Profile.icc -sprofile:ISOcoated_v2_eci.icc > '.$to.'';
		}
...
function getBBoxRemote( $file, $path, $box = 'mediabox' ) {
	$data["pdf"] = file_get_contents( $file );
	$data["box"] = $box;
	...
	$rustart = microtime(true);
	$url = 'http://'.RENDERIP.'/dynAPI/tracker/getbbox.php';
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers );
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST" );
...
function getBBoxMeasure( $file, $path, $box = 'mediabox' ) {
	...
	include('/var/www/intra/engine/config.inc.php');
...
function PdfToImageRender( $file, $temp_path, $tempFile ) {
	...
	error_log( "fájl importálás: ".$file );
	$pdf->OpenImportFile( $file, dynapdf::ptOpen, NULL );
	$pdf->ImportPDFFile( 1, 1.0, 1.0 );	
	$pdf->CloseImportfile();
...
function pdftoimage( $file, $page, $settings = array() ) {
	$pdf = new dynapdf();
...
function sql_update( $table, $condition, $where ) {
...
function sql_aget( $table, $condition, $collect ) {
		$datas = array();
...
function sql_add( $table, $names, $datas ) {
