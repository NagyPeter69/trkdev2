...
function arrowChecker( $id, $page ) {
	...
	if( !empty( $prev[0]["id"] ) ) {
		if( empty( $next[0]["id"] ) ) {
			return "<div class='planner-arrow-right'><div class='arrow-right'></div></div>
			<div onclick='settingsPanel(\"flatplan_worker\", undefined, \"".$current[0]["id"]."\" )' data-id='".$current[0]["id"]."' class='fp-user-icon'><i class='fp-icons fas fa-user'></i></div>
			<div onclick='showAssets(\"".$id."\", \"".$current[0]["id"]."\" )' data-id='".$current[0]["id"]."' class='fp-cog-icon'><i class='fp-icons fas fa-archive' style='color: ".$color.";'></i></div>";
			}
		else {
			return "<div class='planner-arrow-no'></div>";
			}
		}
	else {
		if( empty( $next[0]["id"] ) ) {
			...
			return "<div class='planner-arrow-left'><div class='arrow-left'></div></div>";
			}
		}
	}
...
function invoicing( $pubID ) {
	...
	$proof = 0;
	$coverProof = 0;
	$temp = sql_aget( "pageinfo", "code='".$magazine[0]["code"]."' AND status='2' AND fin='1' AND issue='".$pub[0]["code"]."' AND ( type='ad' or type='magazine') GROUP BY page", "*" );
...
function sendMail( $subject, $body, $to, $cc ) {
	if( is_file( "/var/www/intra/client/plugins/swiftmailer/lib/swift_required.php" ) ) {
		require_once( "/var/www/intra/client/plugins/swiftmailer/lib/swift_required.php" );
		}
	else {
		return 0;
		}
		
	$mail_settings = sql_aget( "tracker_settings", "1 order by `id` ASC", "*" );	
	$licence = sql_get( "tracker_settings", "name='system_type'", "*" );
	$from = array( $mail_settings[13]["value"] => "Colorcom Tracker" );
	$to_temp = explode( "|", $to );
	$to_ = array(
		$to_temp[1] => $to_temp[0]
		);
	...
	$html = $body;
	try {
		$transport = Swift_SmtpTransport::newInstance( $mail_settings[6]["value"], 25 );	
		$transport->setUsername( $mail_settings[10]["value"] );
		$transport->setPassword( $mail_settings[11]["value"] );
		
		$transport->setTimeout(10);
		$swift = Swift_Mailer::newInstance($transport);
		$swift->getTransport()->start();
...
function colorPick( $pdfPath, $x, $y ) {
	$data["pdf"] = file_get_contents( $pdfPath );
	$data["x"] = $x;
	$data["y"] = $y;
	...
	$url = 'http://'.RENDERIP.'/r3API/colorpick.php';
	$ch = curl_init();
...
function getBBox( $file, $path, $box = 'mediabox' ) {
	...
	error_log( "próbálom megnyitni: ".$file );
